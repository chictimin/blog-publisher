...
배포 URL: `https://chictimin.github.io/blog-publisher/` <!-- 아직 미배포. 배포 후 주석 제거 -->

## 무엇을 하는 앱인가

- 입력: 마크다운 초안 전문, 대상 저장소(owner/repo), 방문자가 직접 입력한 Anthropic API 키와 GitHub PAT.
- 결과물: 대상 저장소 `content/blog/<slug>.md`를 추가하는 PR 1건. `main`에 직접 push하지 않고 PR까지만 만든다.

## 준비할 것

(a) **Anthropic API 키** — https://console.anthropic.com 에서 발급. 모델 목록 조회와 실행에 사용한다.
...
2. 모델 선택 — 목록은 키로 조회하며 기본 선택 없음. 고르지 않으면 실행 불가.
3. 초안 입력 — 마크다운 붙여넣기.
...
- 키와 토큰은 브라우저 탭 메모리에만 둔다. `localStorage`·`sessionStorage`에 저장하지 않으며, 새로고침하면 사라지고 다시 입력해야 한다.
- 브라우저에서 Anthropic API를 직접 호출하므로 키가 클라이언트 코드에 노출된다는 위험이 있다. Anthropic 공식 문서에도 이 위험을 경고하는 문구가 있다. 그래서 타인의 키가 아닌 방문자 본인의 키로만 동작하도록 범위를 한정했다.
