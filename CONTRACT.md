...
7. **사용법 모달** — 헤더의 "사용법" 버튼으로 열고 닫는다. 처음 온 방문자가 무엇을 준비해야 하는지 화면을 떠나지 않고 알 수 있게 한다. 담을 내용: 준비물(API 키·baseUrl 예시·GitHub PAT의 필요 권한과 발급 링크), 사용 순서, 승인 게이트가 무엇을 하고 무엇을 하지 않는지(PR만 생성·머지 안 함), 키를 메모리에만 두므로 새로고침 시 사라진다는 점, 한계. **자동으로 띄우지 않는다** — "이미 봤음" 상태를 저장하려면 브라우저 저장소가 필요한데 계약이 그걸 금지한다.
